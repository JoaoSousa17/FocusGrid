...
import { X, UserPlus, Mail, Shield, Eye, Trash2, Clock, Infinity, Check, Loader2, Users } from "lucide-react";
  ...
  const addShare = async () => {
    ...
    try {
      ...
      const { error: err } = await supabase.from("task_shares").upsert(
        { shared_with_email: email.trim().toLowerCase(), role, expires_at: expiresAt },
          ...
          >
            ...
            <div className="flex items-center gap-3 px-5 pt-5 pb-4 border-b border-border">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-muted-foreground">Adiciona pessoas para verem ou editarem as tuas tarefas</p>
              </div>
            ...
            <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto">
              ...
              <div className="space-y-3">
                <label className="text-xs font-semibold text-foreground uppercase tracking-wide">Adicionar pessoa</label>

                {/* Email input */}
                <div className="flex items-center gap-2 border border-border rounded-2xl px-3 py-2.5 focus-within:border-[#E87A5A]/60 transition-all">
                ...
                <div className="flex gap-2">
                  {[
                    { key: "viewer", icon: Eye, label: "Visualizador", desc: "Pode ver mas não editar" },
                    { key: "editor", icon: Shield, label: "Editor", desc: "Pode criar e editar tarefas" },
                  ].map((r) => (
                    <button
                      key={r.key}
                      ...
                      className={`flex-1 flex flex-col items-start gap-1 px-3 py-2.5 rounded-xl border text-left transition-all ${
                        role === r.key
                          ...
                          : "border-border hover:border-[#E87A5A]/40"
                      }`}
                    >
                      <div className="flex items-center gap-1.5">
                        <r.icon className={`w-3.5 h-3.5 ${role === r.key ? "text-[#E87A5A]" : "text-muted-foreground"}`} />
                        <span className={`text-xs font-semibold ${role === r.key ? "text-[#E87A5A]" : "text-foreground"}`}>{r.label}</span>
                      </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1.5 flex-1">
                    ...
                    <input
                      ...
                      disabled={indefinite}
                      className="w-14 px-2 py-1 rounded-lg border border-border text-xs text-center font-medium disabled:opacity-40 focus:outline-none focus:border-[#E87A5A]"
                    />
                    <span className="text-xs text-muted-foreground">semana{weeks !== 1 ? "s" : ""}</span>
                  </div>
                  <button
                    onClick={() => setIndefinite(!indefinite)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-medium transition-all ${
                      indefinite ? "border-[#E87A5A] bg-[#E87A5A]/5 text-[#E87A5A]" : "border-border text-muted-foreground hover:border-[#E87A5A]/40"
                    }`}
                  >
                ...
                <button
                  ...
                  className="w-full py-2.5 rounded-2xl bg-[#E87A5A] text-white text-sm font-semibold flex items-center justify-center gap-2 hover:bg-[#D4694A] transition-all disabled:opacity-50"
                >
                  {adding
                    ? <><Loader2 className="w-4 h-4 animate-spin" /> A partilhar…</>
                    : <><UserPlus className="w-4 h-4" /> Partilhar</>}
                </button>
              </div>
              ...
              {shares.length > 0 && (
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-foreground uppercase tracking-wide">Com acesso</label>
                  {loading ? (
                    ...
                    shares.map((s) => {
                      ...
                      return (
                        <div key={s.id} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border ${expired ? "border-rose-100 bg-rose-50/50 opacity-60" : "border-border bg-secondary/30"}`}>
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-medium text-foreground truncate">{s.shared_with_email}</p>
                          ...
                          <select
                            ...
                            className="text-[10px] font-medium px-2 py-1 rounded-lg border border-border bg-white focus:outline-none focus:border-[#E87A5A] cursor-pointer"
                          >
                            <option value="viewer">Ver</option>
                            <option value="editor">Editar</option>
                          </select>
                          <button onClick={() => removeShare(s.id)} className="w-7 h-7 rounded-lg hover:bg-rose-50 hover:text-rose-500 flex items-center justify-center transition-all">
